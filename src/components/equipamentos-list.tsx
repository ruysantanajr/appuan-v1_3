"use client";

import { useState, useTransition } from "react";
import Link from "next/link";
import { criarEquipamento } from "@/app/actions/equipamento";
import type { Tables } from "@/lib/supabase/types";

type EqRow = Tables<"equipamento">;

const STATUS_STYLE: Record<string, { bg: string; color: string }> = {
  ativo:          { bg: "#DCFCE7", color: "#15803D" },
  inativo:        { bg: "#F3F4F6", color: "#6B7280" },
  em_manutencao:  { bg: "#FEF3C7", color: "#92400E" },
};

const STATUS_LABEL: Record<string, string> = {
  ativo: "Ativo", inativo: "Inativo", em_manutencao: "Em manutenção",
};

export default function EquipamentosList({ equipamentos }: { equipamentos: EqRow[] }) {
  const [modal, setModal] = useState(false);
  const [erro, setErro] = useState<string | null>(null);
  const [isPending, startTransition] = useTransition();

  function handleCriar(e: React.FormEvent<HTMLFormElement>) {
    e.preventDefault();
    setErro(null);
    const fd = new FormData(e.currentTarget);
    startTransition(async () => {
      const res = await criarEquipamento(fd);
      if (res.erro) { setErro(res.erro); return; }
      setModal(false);
    });
  }

  return (
    <div className="px-6 py-8">
      <div className="mb-4 flex items-center justify-between">
        <h2 className="text-sm font-semibold text-fg-1">Equipamentos</h2>
        <button
          onClick={() => setModal(true)}
          className="rounded-md px-3 py-1.5 text-xs font-medium text-white transition-colors"
          style={{ background: "#3a1165" }}
          onMouseOver={e => (e.currentTarget.style.background = "#7C3AED")}
          onMouseOut={e => (e.currentTarget.style.background = "#3a1165")}
        >
          + Novo equipamento
        </button>
      </div>

      <div className="overflow-hidden rounded-lg border" style={{ borderColor: "#E9DDF5" }}>
        <table className="w-full text-sm">
          <thead>
            <tr style={{ background: "#F5F0FA" }}>
              <th className="px-4 py-2.5 text-left text-xs font-semibold text-fg-3">Nome</th>
              <th className="px-4 py-2.5 text-left text-xs font-semibold text-fg-3">Tag</th>
              <th className="px-4 py-2.5 text-left text-xs font-semibold text-fg-3">Tipo</th>
              <th className="px-4 py-2.5 text-left text-xs font-semibold text-fg-3">Nº Série</th>
              <th className="px-4 py-2.5 text-left text-xs font-semibold text-fg-3">Status</th>
            </tr>
          </thead>
          <tbody>
            {equipamentos.length === 0 && (
              <tr>
                <td colSpan={5} className="px-4 py-8 text-center text-xs text-fg-3">
                  Nenhum equipamento cadastrado
                </td>
              </tr>
            )}
            {equipamentos.map((eq, i) => {
              const ss = STATUS_STYLE[eq.status] ?? STATUS_STYLE.inativo;
              const nome = [eq.tipo, eq.marca].filter(Boolean).join(" ");
              return (
                <tr
                  key={eq.id}
                  className="border-t"
                  style={{ background: i % 2 === 0 ? "#FFFFFF" : "#faf5ff" }}
                >
                  <td className="px-4 py-2.5">
                    <Link
                      href={`/equipamentos/${eq.id}`}
                      className="text-xs font-medium text-fg-1 underline-offset-2 hover:underline"
                    >
                      {nome || "—"}
                    </Link>
                  </td>
                  <td className="px-4 py-2.5 font-mono text-xs text-fg-2">{eq.tag}</td>
                  <td className="px-4 py-2.5 text-xs text-fg-3">{eq.tipo}</td>
                  <td className="px-4 py-2.5 text-xs text-fg-3">{eq.numero_serie ?? "—"}</td>
                  <td className="px-4 py-2.5">
                    <span
                      className="rounded-full px-2 py-0.5 text-[10px] font-medium"
                      style={{ background: ss.bg, color: ss.color }}
                    >
                      {STATUS_LABEL[eq.status] ?? eq.status}
                    </span>
                  </td>
                </tr>
              );
            })}
          </tbody>
        </table>
      </div>

      {modal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div
            className="w-full max-w-md rounded-xl border p-6 shadow-xl"
            style={{ background: "#FFF", borderColor: "#E9DDF5" }}
          >
            <h3 className="mb-4 text-sm font-semibold text-fg-1">Novo equipamento</h3>
            <form onSubmit={handleCriar} className="flex flex-col gap-3">
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="mb-1 block text-[11px] text-fg-3">Tipo *</label>
                  <input
                    name="tipo"
                    required
                    className="w-full rounded-md border px-3 py-2 text-xs outline-none"
                    style={{ borderColor: "#E9DDF5" }}
                    placeholder="Ex.: Notebook"
                  />
                </div>
                <div>
                  <label className="mb-1 block text-[11px] text-fg-3">Marca</label>
                  <input
                    name="marca"
                    className="w-full rounded-md border px-3 py-2 text-xs outline-none"
                    style={{ borderColor: "#E9DDF5" }}
                    placeholder="Ex.: Dell"
                  />
                </div>
                <div>
                  <label className="mb-1 block text-[11px] text-fg-3">Tag *</label>
                  <input
                    name="tag"
                    required
                    className="w-full rounded-md border px-3 py-2 font-mono text-xs outline-none"
                    style={{ borderColor: "#E9DDF5" }}
                    placeholder="Ex.: NB-001"
                  />
                </div>
                <div>
                  <label className="mb-1 block text-[11px] text-fg-3">Nº Série</label>
                  <input
                    name="numero_serie"
                    className="w-full rounded-md border px-3 py-2 text-xs outline-none"
                    style={{ borderColor: "#E9DDF5" }}
                  />
                </div>
              </div>
              <div>
                <label className="mb-1 block text-[11px] text-fg-3">Descrição</label>
                <textarea
                  name="descricao"
                  rows={2}
                  className="w-full rounded-md border px-3 py-2 text-xs outline-none"
                  style={{ borderColor: "#E9DDF5" }}
                />
              </div>
              {erro && <p className="text-[11px] text-red-600">{erro}</p>}
              <div className="flex justify-end gap-2 pt-1">
                <button
                  type="button"
                  onClick={() => setModal(false)}
                  className="rounded-md px-3 py-1.5 text-xs text-fg-3 hover:text-fg-1"
                >
                  Cancelar
                </button>
                <button
                  type="submit"
                  disabled={isPending}
                  className="rounded-md px-4 py-1.5 text-xs font-medium text-white disabled:opacity-50"
                  style={{ background: "#3a1165" }}
                >
                  {isPending ? "Salvando…" : "Salvar"}
                </button>
              </div>
            </form>
          </div>
        </div>
      )}
    </div>
  );
}
