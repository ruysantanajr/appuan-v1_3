...
export async function criarOrganizacao(formData: FormData) {
  ...
  const { data, error } = await insertRow(supabase, "organizacao", payload);
  if (error) return { erro: error.message };
  if (!data) return { erro: "Erro ao criar organização." };
...
export async function atualizarOrganizacao(
  ...
  const { data, error } = await updateRow(supabase, "organizacao", payload, id);
  if (error) return { erro: error.message };
...
export async function alterarStatusOrganizacao(
  id: string,
  novoStatus: string,
  dadosAntes: Record<string, unknown>
) {
